...
export function TagPicker({
  ...
  const suggestions = (
    result.data?.tags ||
    SUGGESTED_TAGS.filter((t) => t.includes(normalized)).map((tag) => ({
      tag,
      count: 0,
    }))
  ).slice(0, 12);
  const create =
    validTag(normalized) &&
    !suggestions.some((t) => t.tag === normalized) &&
      ...
      <PopoverContent className="tag-popover" align="start" sideOffset={12}>
        ...
        <Input
          ...
          onKeyDown={(e) => {
            if (e.key === 'Enter') {
              e.preventDefault();
              if (validTag(normalized)) choose(normalized);
            }
          }}
        />
        {value.length > 0 && (
          <div className="tag-options selected-tags">
            {value.map((t) => (
              <button type="button" key={t} onClick={() => choose(t)}>
                #{t}
                <X size={12} />
        ...
        {query && !validTag(normalized) && (
          <p className="tag-hint">Use 2–24 letters or numbers.</p>
        )}
        {result.isError && (
          <p className="tag-hint">
            Suggested tags unavailable. You can still add yours.
          </p>
        )}
        <button
          type="button"
