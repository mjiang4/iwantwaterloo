...
function Plant({idea,index,selected,onSelect,motion}:{idea:Idea;index:number;selected:boolean;onSelect:()=>void;motion:boolean}){
 const ref=useRef<THREE.Group>(null),[hover,setHover]=useState(false),scale=useRef(.03),velocity=useRef(0),[x,z]=spots[(idea.plot??index)%spots.length];
 ...
 <Tree x={0} z={0} n={(idea.plot??index)+7}/>
 <Html position={[0,2.15,0]} center zIndexRange={[20,0]}><button className={`plant-marker ${selected?'selected':''} ${idea.example?'example-tree':''}`} aria-label={`${idea.example?'Example':'Read idea'}: ${idea.title}`} onClick={e=>{e.stopPropagation();onSelect();}} onFocus={()=>setHover(true)} onBlur={()=>setHover(false)}>{idea.example?'E':String((idea.plot??index)-5)}<span className="plant-tooltip">{idea.title}<small>{ideaTags(idea).map(t=>`#${t}`).join(" ")}</small></span></button></Html></group></group>;
}
