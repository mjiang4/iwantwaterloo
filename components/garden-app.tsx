...
import { BarChart3, Download, Heart, Info, Search, SlidersHorizontal, Sprout, X } from 'lucide-react';
...
import { CONNECTIONS, EXAMPLES, ideaTags, filterIdeas, SUGGESTED_TAGS, type Idea } from '@/lib/garden';
...
export type PlantInput = { title: string; description: string; category?: string; tags?: string[]; place: string; connection: string; consent: boolean; website?: string };
type Page = { ideas: Idea[]; examples: Idea[]; total: number; nextPage: number | null };
type Stats = { ideas: number; browsers: number; waters: number; tags: { tag: string; count: number }[]; connections: { connection: string; count: number }[] };
...
function IdeaCard({ idea, onRead, onSupport, pending }: { idea: Idea; onRead: (idea: Idea) => void; onSupport: (idea: Idea) => void; pending: boolean }) {
  return <article className="idea-card">
    <button className="idea-open" onClick={() => onRead(idea)}><span className="idea-topic">{ideaTags(idea).map(t=>`#${t}`).join(" ")||"Idea"}</span><h3>{idea.title}</h3></button>
...
function Overview({ onTag }: { onTag: (tag: string) => void }) {
  ...
  return <section className="overview"><div className="overview-heading"><h1>Overview</h1><a className="quiet-button" href="/api/export"><Download size={16} />Export</a></div>
    {stats.isError ? <p className="form-error">Couldn’t load. <button onClick={() => stats.refetch()}>Retry</button></p> : <>
      ...
      <div className="overview-columns"><section><h2>Tags</h2>{s?.tags?.length?s.tags.map(t => <button className="overview-row" key={t.tag} onClick={() => onTag(t.tag)}><span>#{t.tag}</span><span>{t.count}</span></button>):<p className="empty-copy">None yet.</p>}</section><section><h2>Connections</h2>{s?.connections.length ? s.connections.map(c => <div className="overview-row" key={c.connection}><span>{c.connection}</span><span>{c.count}</span></div>) : <p className="empty-copy">None yet.</p>}</section></div>
    </>}
...
function Garden() {
  ...
  const list = useInfiniteQuery({ queryKey: ['ideas', tag, debouncedQuery, connection, sort], initialPageParam: 0, queryFn: ({ pageParam }) => api<Page>(`/api/ideas?${new URLSearchParams({ tag, q: debouncedQuery, connection, sort, page: String(pageParam) })}`), getNextPageParam: page => page.nextPage, staleTime: 15000, refetchInterval: 20000 });
  const ideas = useMemo(() => {
    const real = list.data?.pages.flatMap(p => p.ideas) || [];
    const examples = list.data?.pages[0].examples || (!list.data ? filterIdeas(EXAMPLES, 'all', debouncedQuery, connection, sort, tag) : []);
    return filterIdeas([...real, ...(showExamples ? examples : [])], 'all', debouncedQuery, connection, sort, tag);
  }, [list.data, showExamples, tag, debouncedQuery, connection, sort]);
  const total = (list.data?.pages[0].total || 0) + (showExamples ? (list.data?.pages[0].examples.length ?? filterIdeas(EXAMPLES, 'all', debouncedQuery, connection, 'newest', tag).length) : 0);
  const filtered = tag !== 'all' || connection !== 'all' || sort !== 'newest' || !showExamples;
  const clearFilters = useCallback(() => { setTag('all'); setTagSearch(''); setQuery(''); setConnection('all'); setSort('newest'); setShowExamples(true); }, []);
  ...
  const share = useCallback(async (input: PlantInput) => {
    ...
    clearFilters();
    setView('ideas');
    await Promise.all([client.invalidateQueries({ queryKey: ['ideas'] }), client.invalidateQueries({ queryKey: ['stats'] }), client.invalidateQueries({ queryKey: ['tags'] }), client.invalidateQueries({ queryKey: ['garden'] })]);
    return idea;
  }, [client, clearFilters]);
  ...
  const support = useCallback(async (idea: Idea) => {
    ...
    try {
      ...
      patchIdea(data.id, data); void client.invalidateQueries({ queryKey: ['stats'] }); void client.invalidateQueries({queryKey:['garden']});
  ...
  useGardenTools({ plant: async input => { const idea = await share(input); setSelected(idea); return idea; }, explore: (q, c) => { setQuery(q); setDebouncedQuery(q); setTag(c); setSearchOpen(Boolean(q)); setView('ideas'); } });
  ...
  function showGarden(idea?:Idea) { clearFilters(); setGardenFocus(idea||null); setView('garden'); requestAnimationFrame(()=>document.getElementById('garden-view')?.scrollIntoView({behavior:reduced?'instant':'smooth',block:'start'})); }
  ...
  return <TooltipProvider delay={250}><Tabs value={view} onValueChange={v => setView(String(v))} className="garden-app">
    ...
    <main>
      <div hidden={view === 'insights'}><IdeaComposer onShare={share} onPrivacy={() => setAboutOpen(true)} onGarden={showGarden} /></div>
      <div className="explore-section">
        ...
        {filtered && view !== 'insights' && <div className="active-filter"><span>{tag === 'all' ? 'Filtered' : `#${tag}`}</span><button aria-label="Clear filters" onClick={clearFilters}><X size={13} /></button></div>}
        ...
        <TabsContent value="ideas" className="view-panel"><div className="browse-tags" aria-label="Browse tags">{(browseTags.data?.tags||SUGGESTED_TAGS.map(tag=>({tag,count:0}))).slice(0,8).map(t=><button type="button" key={t.tag} aria-pressed={tag===t.tag} onClick={()=>setTag(v=>v===t.tag?"all":t.tag)}>#{t.tag}</button>)}</div><section className="ideas-grid" aria-label="Ideas for Waterloo">{ideas.map(idea => <IdeaCard key={idea.id} idea={idea} onRead={selectIdea} onSupport={support} pending={pending.has(idea.id)} />)}{!ideas.length && <div className="empty-state"><p>No ideas yet.</p><Button variant="ghost" onClick={filtered || query ? clearFilters : focusComposer}>{filtered || query ? 'Clear filters' : 'Add the first'}</Button></div>}</section>{list.hasNextPage && <Button variant="ghost" className="load-more" onClick={() => list.fetchNextPage()} disabled={list.isFetchingNextPage}>{list.isFetchingNextPage ? 'Loading…' : 'More ideas'}</Button>}</TabsContent>
        <TabsContent value="garden" className="view-panel" id="garden-view"><GardenExplorer tag={tag} query={debouncedQuery} connection={connection} examples={showExamples} focusIdea={gardenFocus} onRead={selectIdea} onList={()=>setView('ideas')}/></TabsContent>
        <TabsContent value="insights" className="view-panel"><Overview onTag={c => { clearFilters(); setTag(c); setView('ideas'); }} /></TabsContent>
      </div>
    </main>
    <Sheet open={!!selected} onOpenChange={open => { if (!open) setSelected(null); }}><SheetContent side={small ? 'bottom' : 'right'} className="idea-sheet">{selected && <div className="idea-detail"><span className="detail-topic">{ideaTags(selected).map(t=><button type="button" className="detail-tag" key={t} onClick={()=>{clearFilters();setTag(t);setSelected(null);setView("ideas");}}>#{t}</button>)}{selected.example && <span className="example-badge">Example</span>}</span><SheetTitle className="detail-title">{selected.title}</SheetTitle><SheetDescription className="detail-meta">{selected.place || 'Waterloo'}{selected.connection ? ` · ${selected.connection}` : ''}</SheetDescription>{selected.description !== selected.title && <p className="detail-body">{selected.description}</p>}<div className="detail-actions"><SupportButton idea={selected} onSupport={support} pending={pending.has(selected.id)} large /></div>{supportError && <p className="form-error" role="alert">{supportError}</p>}</div>}</SheetContent></Sheet>
