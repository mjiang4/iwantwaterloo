...
- The composer contains only the idea, optional signature and Post action. Place, connection and topic entry are removed; submissions send empty metadata rather than silently using old draft fields. Existing stored tags remain available for browsing. Classification is deferred.
