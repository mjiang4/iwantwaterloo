...
Preview is the actual application, compiled from the current working copy with the production build command. The visitor experience is the same application. `/admin` now redirects to the focused `/organizer` review desk in preview; it remains unavailable in production. See [participation](PARTICIPATION.md) for the review workflow.
