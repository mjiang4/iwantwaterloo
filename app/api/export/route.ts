...
import {categoryFor} from '@/lib/garden';
...
export async function GET(request:Request){const {id}=identity(request);try{
 ...
 controller.enqueue(encoder.encode('\uFEFF'+['ID','Title','Idea','Theme','Place','Connection to Waterloo','Planted at (UTC)','Waters'].map(csv).join(',')+'\r\n'));
 ...
 for(const i of result.results){controller.enqueue(encoder.encode([i.id,i.title,i.description,categoryFor(String(i.category)).label,i.place,i.connection,new Date(Number(i.created_at)).toISOString(),i.waters].map(csv).join(',')+'\r\n'));lastCreated=Number(i.created_at);lastId=String(i.id);}if(result.results.length<100)break;}
